...
class CAMSProvider:
    ...
    def _load_cams_data(self, obs_time: datetime) -> xr.Dataset | None:
        """Load CAMS data for the observation date."""
        source_is_remote = self._is_remote_source(self.data_dir)
        source_is_explicit = self._is_explicit_source(self.data_dir)
        ...
        if not data_dir.exists():
            if self.download_missing:
                ...
                return self._load_cds_dataset(obs_time)
            else:
        ...
        patterns = self._netcdf_patterns(date_str, iso_date)
        ...
        if local_candidates_found or self._has_cams_tif_candidates(date_str, iso_date):
            return None
    ...
    @classmethod
    def _is_explicit_source(cls, value: str | Path) -> bool:
        return cls._source_suffix(value) in cls._NETCDF_SUFFIXES | cls._TIFF_SUFFIXES
    ...
    @staticmethod
    def _tif_patterns(date_str: str, iso_date: str) -> list[str]:
        return [
            f"*{date_str}*.tif*",
            f"*{iso_date}*.tif*",
            "*.tif",
