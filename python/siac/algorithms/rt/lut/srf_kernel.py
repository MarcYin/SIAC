"""LUT-aligned kernels derived from canonical spectral response functions."""
...
class AlignedSRFKernel:
    """SRF interpolated onto a specific LUT wavelength axis slice."""
...
def build_aligned_srf_kernel(
    ...
    *,
    ...
    """Interpolate an SRF onto the active LUT wavelength axis and trim to support."""
    ...
    support_mask = (wavelengths >= srf.support_min_nm) & (wavelengths <= srf.support_max_nm)
    if np.any(support_mask):
        ...
        left = max(int(np.searchsorted(wavelengths, srf.support_min_nm, side="left")) - support_padding, 0)
        right = min(int(np.searchsorted(wavelengths, srf.support_max_nm, side="right")) + support_padding, wavelengths.size)
        if left == right:
    ...
    if response_area <= 0.0:
        raise ValueError("Aligned SRF support has zero overlap with LUT wavelength axis")
    ...
    return AlignedSRFKernel(
        sensor_id=srf.sensor_id,
        satellite_id=srf.satellite_id,
        band_name=srf.band_name,
        lut_id=lut_id,
